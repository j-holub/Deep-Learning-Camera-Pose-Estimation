...
def Network(input_tensor):
    ...
    fc3_weights = tf.Variable(tf.random_normal([64, 3], dtype=tf.float64) / np.sqrt(64), dtype=tf.float64)
    fc3_bias    = tf.Variable(tf.zeros([3], dtype=tf.float64), dtype=tf.float64)
